...
const Card = (data) => {
    ...
    const renderIcon = (id) => {
      const isInCart =
        context.carProduct.filter((product) => product.id == id).length > 0;

      if (isInCart) {
        return (
            <div
              className="absolute top-0 right-0 flex justify-center items-start bg-black w-6 h-6 rounded-full m-2 p-1"
            >
